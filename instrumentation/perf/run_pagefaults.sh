...
#
# run_pagefaults.sh — show that the axon RT path takes ~0 page faults in steady
...
DEMO="${REPO}/build/examples/demo/axon_demo"
...
run() {  # $1=frames
    ...
    AXON_CORES=0-3 AXON_TIMEOUT=60 /usr/bin/time -v \
...
echo "axon RT page-fault check (getrusage, no sudo) — faults should NOT scale"
