#!/bin/bash
# In-container demo: roscore + producer + consumer, all on one host so the axon
...
rosrun axon_ros1 producer_node _bytes:="$BYTES" _rate_hz:="$RATE" \
    >/tmp/producer.log 2>&1 &
sleep 1.5

echo "=== descriptor topic is a normal ROS1 topic (watchdog/rosbag/rosgraph work) ==="
( timeout 4 rostopic hz /axon/tensor_desc 2>&1 | sed 's/^/[rostopic hz] /' ) &
( timeout 3 rosbag record -O /tmp/desc.bag /axon/tensor_desc >/dev/null 2>&1 ) &

echo "=== consumer: reads payload zero-copy per descriptor (8s) ==="
timeout --signal=INT 8 rosrun axon_ros1 consumer_node || true

echo "=== rosbag of the DESCRIPTOR topic only (payload never serialized) ==="
ls -la /tmp/desc.bag* 2>/dev/null || echo "(no bag)"
rosbag info /tmp/desc.bag 2>/dev/null | grep -iE "size|messages|topics|axon" || true
