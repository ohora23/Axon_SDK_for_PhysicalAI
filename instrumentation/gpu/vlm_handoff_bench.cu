...
// dczc VLM embedding handoff benchmark (RTX 5080).
//
...
//   dczc   : the buffer is CUDA VMM memory shared once via the dczc SCM_RIGHTS
...
//
// Emits a per-size table: dczc vs naive p50 handoff latency, GB/s, copies.
...
int run_producer(int sock) {
    ...
    if(dczc::detail::send_fds(sock,fds,2,&m,sizeof(m))!=0){ perror("send_fds"); return 30; }
    ...
    for (auto& e : SIZES) {
        ...
        for (int f=1; f<=N_FRAMES; ++f) {
            ...
            // --- dczc: publish a 32B commit; buffer already visible in-place ---
...
int run_consumer(int sock) {
    CKR(cudaSetDevice(0)); CKR(cudaFree(0));
    int fds[dczc::detail::kMaxSidecarFds]; int n=0; Meta m{};
    if(dczc::detail::recv_fds(sock,fds,dczc::detail::kMaxSidecarFds,&n,&m,sizeof(m))!=0||n!=2)
    ...
    printf("\n────── dczc VLM embedding handoff (RTX 5080, %d frames/size) ──────\n", N_FRAMES);
    printf("  %-20s %10s %10s %8s   %10s %8s\n",
           "embedding", "dczc p50", "naive p50", "speedup", "naive GB/s", "copies");

    for (auto& e : SIZES) {
        uint64_t nel=e.bytes/sizeof(uint64_t), blocks=(nel+threads-1)/threads;
        std::vector<uint64_t> dczc_us, naive_us;
        uint64_t corrupt=0;
        for (int f=1; f<=N_FRAMES; ++f) {
            char go;
            // dczc: buffer already shared; read commit, verify in place, done.
            ...
            if(mism) ++corrupt;
            dczc_us.push_back(now_ns()-commit_host->ts_ns);
        ...
        double d=p50(dczc_us), na=p50(naive_us);
    ...
    printf("──────────────────────────────────────────────────────────────────\n");
    printf("  dczc: buffer shared once via SCM_RIGHTS sidecar, read in place (0 host copies)\n");
...
int main() {
    ...
    fprintf(stderr,"VLM handoff bench: dczc zero-copy vs naive host round-trip\n");
    pid_t pid=fork();
