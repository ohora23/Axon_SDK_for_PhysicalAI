...
# Override via env: AXON_CORES, AXON_MEM, AXON_TIMEOUT, AXON_CPUQUOTA.
#
# Usage:  instrumentation/run_bounded.sh <command> [args...]
#         AXON_CORES=0-3 AXON_MEM=4G AXON_TIMEOUT=30 run_bounded.sh python3 x.py

set -euo pipefail

CORES="${AXON_CORES:-0-5}"          # 6 of 16 cores by default
MEM="${AXON_MEM:-8G}"
TIMEOUT="${AXON_TIMEOUT:-120}"
CPUQUOTA="${AXON_CPUQUOTA:-600%}"   # cgroup ceiling (belt-and-suspenders w/ taskset)
...
if systemd-run --user --scope -q -p Description="axon-bounded" true >/dev/null 2>&1; then
