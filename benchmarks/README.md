# axon vs ROS2 latency benchmark (design doc §8.2)

Quantifies the project's central thesis: ROS2's DDS path serializes and copies the
**entire payload** on every frame, while axon sends only a fixed-size descriptor
through its metadata queue and keeps the tensor in a shared dma-buf. So axon
...
- **axon** — `bench_axon.py`: producer/consumer processes over the real sidecar +
...
- **ROS2** — `bench_ros2.py`: `UInt8MultiArray` over the default Fast-RTPS DDS, QoS
  `sensor_data` (best-effort, keep-last depth 1) to mirror axon's latest-value
  seqlock semantics. Publisher and subscriber are spawned as separate processes.

## Run

```bash
# Build the axon Python module first
...
be run directly (`bench_axon.py`, `bench_ros2.py` — the latter needs
...
|---|---|---|---|---|
| axon (zero-copy) | 18.4 | 30.6 | 48.2 | 583 |
...
|---|---|---|---|---|---|
| axon (zero-copy) | 46.4 | 76.3 | 237.9 | 4981 | 300 / 300 |
| ROS2 (Fast-RTPS) | 970.6 | 1103.0 | 1272.5 | 1290 | 291 / 300 |
| **speedup** | **20.9×** | | **5.3×** | | |

### Takeaways

1. **axon is ~9–20× lower p50 latency** across payload sizes.
2. **Payload scaling is the story.** From 147 KiB → 1 MiB, axon's p50 grew ~2.5×
...
3. **Under load ROS2 dropped frames** (291/300, best-effort DDS); axon delivered
