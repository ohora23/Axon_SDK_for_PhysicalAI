# MockSystem — multi-stream robot workload (axon vs ROS2)
...
> so its cost scales with bandwidth. axon sends only a fixed-size descriptor
...
- **axon side** (`mock_axon.py`): one publisher per stream; a single **1 kHz RT
  loop** reads the latest of every stream each tick (the axon usage model, design
...
(`mock_compare.py` runs axon against the built module and ROS2 in a sourced
...
|---|---|---|---|---|---|---|---|
| 1 | 74 MB/s | axon | 74 | 1.000 | 1.000 | **0.26** | 578 / 1073 |
| 1 | 74 MB/s | ROS2 | 73 | 0.998 | 0.992 | 0.77 | 636 / 851 |
| 2 | 148 MB/s | axon | 148 | 1.000 | 1.000 | **0.28** | 548 / 1086 |
| 2 | 148 MB/s | ROS2 | 146 | 0.995 | 0.983 | 0.83 | 580 / 976 |
| 3 | 221 MB/s | axon | 221 | 1.000 | 1.000 | **0.29** | 578 / 1084 |
| 3 | 221 MB/s | ROS2 | 218 | 0.993 | 0.967 | 0.88 | 471 / 784 |
| 4 | 295 MB/s | axon | 295 | 1.000 | 1.000 | **0.30** | 597 / 1095 |
| 4 | 295 MB/s | ROS2 | 291 | 0.993 | 0.975 | 0.98 | 508 / 772 |

### Takeaways

1. **CPU: axon is flat, ROS2 scales with bandwidth.** As offered load grew 4×
   (74 → 295 MB/s), axon CPU barely moved (0.26 → 0.30 cores) while ROS2 climbed
   (0.77 → 0.98). At scale 4, **ROS2 uses 3.3× the CPU** for the same data — the
   cost of serializing and copying every payload on both sides. axon wins *despite*
   paying for a 1 kHz polling loop that ROS2's event model doesn't.
2. **Delivery: axon drops nothing; ROS2 drops under load.** axon held 100%
   delivery at every scale; ROS2's worst-stream delivery fell to ~0.967 (frames
   lost to best-effort DDS under load).
3. **Latency is comparable** (~0.5–1.1 ms) — axon's is dominated by the 1 kHz
   control tick, ROS2's by callback scheduling — so the win is in CPU headroom and
   zero drops, not tail latency here.

The headroom is the point: on a robot's shared compute, the CPU axon *doesn't*
