...
#
# run_syscalls.sh — count data-path syscalls for dczc vs ROS2 with strace -f -c.
...
#
# dczc's payload path is pure shared memory: after the one-time FD handshake, a
...
echo "dczc: expect transport syscalls = one-time handshake only (not per frame).\n"

echo "########## dczc ##########"
DCZC_CORES=0-3 DCZC_TIMEOUT=90 "$BOUND" \
    strace -f -c -e "trace=${TR}" \
    env PYTHONPATH="$MOD" python3 "${REPO}/benchmarks/mock/mock_dczc.py" \
...
if [ -n "$SETUP" ]; then
    echo "########## ROS2 (Fast-RTPS) ##########"
    DCZC_CORES=0-3 DCZC_TIMEOUT=120 "$BOUND" \
        bash -lc "source ${SETUP} && strace -f -c -e trace=${TR} \
            python3 ${REPO}/benchmarks/mock/mock_ros2.py --scale ${SCALE} --seconds ${SECS}" \
        2>&1 | summary
fi
echo
echo "dczc frames publish via seqlock (shared memory) — no per-frame syscall;"
